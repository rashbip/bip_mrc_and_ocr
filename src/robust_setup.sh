...
check_sys_dep() {
    # Special case for libxslt-dev which maps to libxslt1-dev on Ubuntu 24.04
    local dep=$1
    if [ "$dep" == "libxslt-dev" ]; then
        if dpkg -s "libxslt1-dev" >/dev/null 2>&1; then
            echo -e "${GREEN}[OK]${NC} System package: libxslt1-dev is already installed."
            return
        fi
    fi

    if dpkg -s "$dep" >/dev/null 2>&1; then
        echo -e "${GREEN}[OK]${NC} System package: $dep is already installed."
    else
        echo -e "System package: $dep is missing. Installing..."
...
# Check if libraries are already installed to avoid redundant pip install
if "$VENV_DIR/bin/python3" -c "import fitz; import internetarchivepdf" >/dev/null 2>&1; then
    echo -e "${GREEN}[OK]${NC} Python libraries already installed."
else
